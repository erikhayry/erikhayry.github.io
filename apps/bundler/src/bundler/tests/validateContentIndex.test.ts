        ...
        describe('panel data', () => {
            test('should throw error on invalid id', () => {
                try {
                    ...
                    expect(error.id).toEqual('NOPE')
                    expect(error.message).toBeDefined()
                    expect(error.path).toEqual(VALID_PANEL_DATA_MOCK_1_1.path)
                }
            })
            ...
            test('should throw error on invalid data', () => {
                try {
                    ...
                    expect(error.message).toBeDefined()
                    expect(error.id).toEqual(VALID_PANEL_DATA_MOCK_1_1.id)
                    expect(error.path).toEqual(VALID_PANEL_DATA_MOCK_1_1.path)
                }
            })
        })
        ...
        describe('comic file', () => {
            ...
            test('should not validate if missing', () => {
                try {
                    ...
                    expect(message.message).toEqual(ErrorMessage[VALIDATION.COMIC_DATA_FILE])
                }
            });
            ...
            test('should not validate if portrait image is missing for comic style AN', () => {
                try {
                    ...
                    expect(message.validation).toEqual(VALIDATION.COMIC_DATA_PORTRAIT_IMAGE_FILE)
                    expect(message.comicStyle).toEqual(ComicStyle.ANIME)
                }
            })
            ...
            test('should not validate if landscape image is missing for comic style AN', () => {
                try {
                    ...
                    expect(message.validation).toEqual(VALIDATION.COMIC_DATA_LANDSCAPE_IMAGE_FILE)
                    expect(message.comicStyle).toEqual(ComicStyle.ANIME)
                }
            })
            ...
            test('should not validate if portrait image is missing for comic style BE', () => {
                try {
                    ...
                    expect(message.message).toEqual(ErrorMessage[VALIDATION.COMIC_DATA_PORTRAIT_IMAGE_FILE])
                    expect(message.validation).toEqual(VALIDATION.COMIC_DATA_PORTRAIT_IMAGE_FILE)
                    expect(message.comicStyle).toEqual(ComicStyle.BELGIAN_COMIC)
                }
            })
            ...
            describe('layout 1', () => {
                test('should not validate if landscape panel is missing for page layout 1', () => {
                    try {
                        ...
                        expect(message.validation).toEqual(VALIDATION.LAYOUT_LANDSCAPE_IMAGE_COUNT)
                        expect(message.comicStyle).toEqual(ComicStyle.ANIME)
                    }
                })
                test('should not validate if portrait panel is missing for page layout 1', () => {
                    try {
                        ...
                        expect(message.validation).toEqual(VALIDATION.LAYOUT_PORTRAIT_IMAGE_COUNT)
                        expect(message.comicStyle).toEqual(ComicStyle.ANIME)
                    }
                })
            });
            ...
            describe('layout 1', () => {
                test('should not validate if panel data is missing for page layout 1', () => {
                    try {
                        ...
                        expect(message.validation).toEqual(VALIDATION.PANEL_DATA_FILE)
                        expect(message.id).toEqual(VALID_PAGE_LAYOUT_1_DATA_MOCK.id)
                    }
                })
            })
            ...
            describe('layout 2', () => {
                test('should not validate if panel data is missing for page layout 2', () => {
                    try {
                        ...
                        expect(message.message).toEqual(ErrorMessage[VALIDATION.PANEL_DATA_FILE])
                        expect(message.validation).toEqual(VALIDATION.PANEL_DATA_FILE)
                        expect(message.id).toEqual(VALID_PAGE_LAYOUT_2_DATA_MOCK.id)
                    }
                })
            });
        })
